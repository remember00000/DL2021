...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        f1_out=np.dot(X,W1)+b1
        f1_out_relu=np.maximum(f1_out,0)
        f2_out=np.dot(f1_out_relu,W2)+b2
        f2_out_relu=np.maximum(f2_out,0)
        scores=np.dot(f2_out_relu,W3)+b3
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            for key_,val_ in self.params.items():
              # print('key_',key_,'val_',val_)
              # print('key_',key_,'self.params[key_]:',self.params[key_])
              # print('grads[key_]:',grads[key_])
              self.params[key_]+=-learning_rate*grads[key_]
